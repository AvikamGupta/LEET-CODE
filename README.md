<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:f9a825,50:f57f17,100:e65100&height=200&section=header&text=LeetCode%20Solutions&fontSize=55&fontColor=ffffff&fontAlignY=35&desc=Solving%20Problems.%20Building%20Intuition.%20Every%20Day.&descAlignY=55&descSize=18"/>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=20&pause=1000&color=FFA726&center=true&vCenter=true&width=700&lines=34+Problems+Solved+and+Counting+%F0%9F%94%A5;Arrays+%7C+Strings+%7C+Math+%7C+DP+%7C+Greedy;Java+%2B+C%2B%2B+Solutions;One+Problem+at+a+Time+%F0%9F%9A%80"/>

<br/>

[![LeetCode](https://img.shields.io/badge/LeetCode-Avikam__Gupta-FFA116?style=for-the-badge&logo=leetcode&logoColor=white)](https://leetcode.com/Avikam_Gupta)
![Problems Solved](https://img.shields.io/badge/Solved-34-brightgreen?style=for-the-badge)
![Languages](https://img.shields.io/badge/Languages-Java%20%7C%20C%2B%2B-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)
![Status](https://img.shields.io/badge/Status-Active%20🔥-orange?style=for-the-badge)

</div>

---

## 📊 Live Stats

<div align="center">

### LeetCode

[![LeetCode Stats](https://leetcard.jacoblin.cool/Avikam_Gupta?theme=dark&font=Fira+Code&ext=heatmap)](https://leetcode.com/Avikam_Gupta)

### GitHub

<img src="https://github-readme-stats.vercel.app/api?username=AvikamGupta&show_icons=true&theme=radical&hide_border=true&include_all_commits=true&count_private=true&title_color=FFA726&icon_color=FFA726&text_color=ffffff&bg_color=0d1117" height="170"/>
&nbsp;&nbsp;
<img src="https://github-readme-streak-stats.herokuapp.com/?user=AvikamGupta&theme=radical&hide_border=true&stroke=FFA726&ring=FFA726&fire=ffffff&currStreakLabel=FFA726&background=0d1117" height="170"/>

</div>

---

## 👨‍💻 About This Repository

A daily-updated collection of my LeetCode solutions written in **Java** and **C++**.

Every solution here reflects my genuine attempt first — then review, debug, and understand. The goal isn't just to pass test cases; it's to deeply understand *why* a solution works.

> *"Solve it. Understand it. Explain it. That's when you've actually learned it."*

---

## 📊 Progress Overview

```
Total Solved   : 34 Problems
Languages      : Java · C++
Consistency    : 🔥 Daily
```

### By Category

| Category | Solved | Progress |
|----------|:------:|----------|
| 📦 Arrays | 15 | `█████████░░░░░░░░░░` |
| 🔤 Strings | 10 | `██████░░░░░░░░░░░░░░` |
| ➗ Mathematics | 9 | `██████░░░░░░░░░░░░░░` |
| 🔗 Linked Lists | 0 | `░░░░░░░░░░░░░░░░░░░░` |
| 🌳 Trees | 0 | `░░░░░░░░░░░░░░░░░░░░` |
| ⚡ Dynamic Programming | 1 | `█░░░░░░░░░░░░░░░░░░░` |
| 🏃 Greedy | 1 | `█░░░░░░░░░░░░░░░░░░░` |
| 🔢 Bit Manipulation | 1 | `█░░░░░░░░░░░░░░░░░░░` |

---

## 🗂️ All Solutions

### 📦 Arrays

| # | Problem | Difficulty | Language | Approach |
|---|---------|:----------:|:--------:|----------|
| 01 | Two Sum | 🟢 Easy | Java | Hash Map |
| 02 | Left and Right Sum Differences | 🟢 Easy | Java | Prefix Sum |
| 03 | Container With Most Water | 🟡 Medium | Java | Two Pointer |
| 04 | Remove Element | 🟢 Easy | Java | Two Pointer |
| 05 | Remove Duplicates from Sorted Array | 🟢 Easy | Java | Two Pointer |
| 06 | Find Index of First Occurrence in a String | 🟢 Easy | Java | Sliding Window |
| 07 | Jump Game | 🟡 Medium | Java | Greedy |
| 08 | Numbers with Even Number of Digits | 🟢 Easy | Java | Iteration |
| 09 | Max Consecutive Ones | 🟢 Easy | Java | Iteration |
| 10 | Maximum Subarray Sum | 🟡 Medium | c++ | Kadane's Algortithm |
| 11 | Majority Elements | 🟢 Easy | c++ | Moores's voting algorithm |
| 12 | Product of array except itself | 🟡 Medium | c++ |  Prefix Suffix Product |
| 13 | Rank Transform | 🟢 Easy | java | Hashmap |
| 14 | Squares of sorted array | 🟢 Easy | Java | Brute force |
| 15 | Running sum of 1-D array | 🟢 Easy | Java | Bruteforce |
---

### 🔤 Strings

| # | Problem | Difficulty | Language | Approach |
|---|---------|:----------:|:--------:|----------|
| 10 | Roman to Integer | 🟢 Easy | Java | Hash Map |
| 11 | Integer to Roman | 🟡 Medium | Java | Greedy |
| 12 | Longest Common Prefix | 🟢 Easy | Java | Sort & Compare |
| 13 | Longest Palindromic Substring | 🟡 Medium | Java | Brute Force + Two Pointer |
| 14 | Excel Sheet Column Title | 🟢 Easy | Java | Math / Base-26 |
| 15 | Single Number | 🟢 Easy | C++ | Bit Manipulation (XOR) |
| 16 | Detect Capital | 🟢 Easy | java | Linear Scan with Counting |
| 17 | Length of last word | 🟢 Easy | java | String Tokenizer |
| 18 | Reverse Words in a string | 🟡 Medium | Java | String tokenizer |
| 19 | Longest substring without repetition | 🟡 Medium | Java | pointers |
---

### ➗ Mathematics

| # | Problem | Difficulty | Language | Approach |
|---|---------|:----------:|:--------:|----------|
| 16 | Palindrome Number | 🟢 Easy | Java | Reversal |
| 17 | Happy Number | 🟢 Easy | Java | Floyd's Cycle |
| 18 | Reverse Integer | 🟡 Medium | Java | Math + Overflow Check |
| 19 | Subtract Product and Sum of Digits | 🟢 Easy | Java | Digit Extraction |
| 20 | Power of Two | 🟢 Easy | Java | Bit Manipulation |
| 21 | Climbing Stairs | 🟢 Easy | Java | Dynamic Programming |
| 22 | pow(x,n) | 🟡 Medium | c++ | Binary Exponentiation |
| 23 | Plus one | 🟢 Easy | Java | Big number arithmetic |
| 24 | GCD of ODD and Even sums | 🟢 Easy | Java | Brute force |
---

## 🌐 Difficulty Breakdown

| Difficulty | Count |
|:----------:|:-----:|
| 🟢 Easy | 24 |
| 🟡 Medium | 10 |
| 🔴 Hard | 0 |

---

## 🛠️ Languages Used

| Language | Problems |
|----------|:--------:|
| ☕ Java | 29 |
| ⚙️ C++ | 5 |

---

## 📌 How I Approach Problems

```
1. Read the problem carefully — understand input/output
2. Think of a brute force solution first
3. Code it up and verify with examples
4. Analyze time & space complexity
5. Optimize if possible
6. Commit & document
```

---

## 🎯 Goals

- ⏳ Solve 100+ Problems
- ⏳ Cover all core DSA topics
- ⏳ Add Linked Lists & Trees
- ⏳ Attempt Medium problems regularly
- ⏳ Start solving Hard problems
- ⏳ Start Competitive Programming

---

<div align="center">

### 👨‍💻 Avikam Gupta

[![LeetCode](https://img.shields.io/badge/Visit%20My%20LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=white)](https://leetcode.com/Avikam_Gupta)

*"The only way to get better at problem solving is to solve more problems."*

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:e65100,50:f57f17,100:f9a825&height=120&section=footer"/>

</div>
